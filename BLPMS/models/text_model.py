...
class CNNStatic(nn.Module):
    def __init__(self, num_classes, embeddim, maxlen, numfilters, filtersizes):
        ...
        self.numfilters = numfilters # default 200
        self.filtersizes = filtersizes # [3, 4, 5]
        self.embeddim = embeddim # bert h_s 768

        builder = Builder()
        
        # 3*3卷积，768 -> 200
        self.conv1 = builder.conv_layer(self.embeddim, self.numfilters, kernel_size=self.filtersizes[0], bias=False)
        #创建卷积层，初始化的时候会初始score和稀疏度
        builder.conv_init(self.conv1)
        #初始化卷积层权重
...
class TextCLModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.numfilters = args.num_filters # default 200
        ...
        if args.emb_model in ['glove', 'fasttext']:
            ...
            self.pretrained_embeddings = AutoModel.from_pretrained("/home/qhn/Codes/Models/Bert-base-uncased")
