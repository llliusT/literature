...
def get_editable_weights_mask_dict(model, type):
    ...
    for n, module in model.named_modules():
        if hasattr(module, 'scores'):
            ...
            if type == "original":#不管不顾，各练各的
                mask_dict[n] = curr_mask
            elif type == "exclusive":#后面不能用前面的                               
                if module.task == 0:
                    mask_dict[n] = curr_mask
                else:
                    for ii in range(module.task):
                        mask_ii = module_util.get_subnet( module.scores[ii].abs(), module.sparsity ).type(torch.bool)
                        temp_mask=curr_mask & mask_ii #当前任务与第i个任务的子网交集
                        if editable_weight_mask is None:
                            editable_weight_mask = (curr_mask ^ temp_mask) #处理第一个任务时，附一个初始值，最终子网=当前任务子网-与第i个任务的交集
                        else:
                            editable_weight_mask = editable_weight_mask & (curr_mask ^ temp_mask)#处理其余任务时，最终子网=最终子网-与第i个任务的交集
                    mask_dict[n] = editable_weight_mask
            elif type == "common":#后面可以用前面的，但是系数不能一样
                if module.task == 0:
                    mask_dict[n] = curr_mask
                else:
                    for ii in range(module.task):
                        ...
                        temp_mask=curr_mask & mask_ii #当前任务与第i个任务的子网交集
                        if ii == 0:
                            overlap_mask = temp_mask
                        else:
                            overlap_mask = overlap_mask | temp_mask
                    remain_mask = curr_mask ^ overlap_mask
                    mask_dict[n] = overlap_mask*args.common_coefficient+remain_mask*args.independence_coefficient
...
def train(model, writer, train_loader, optimizer, criterion, epoch, task_idx, weight_mask_dict, curr_act_dict, ):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if args.iter_lim < 0 or len(train_loader) * (epoch - 1) + batch_idx < args.iter_lim:
            ...
            # print(f"model.named_modules(): {model.named_modules()}")
            for n, m in model.named_modules():
                if hasattr(m, 'scores'):
                    assert (isinstance(m, nn.Conv2d) or isinstance(m, nn.Conv1d)) and m.task == task_idx
                    # for i in range(args.num_tasks): assert m.scores[i].grad is None #判断每个任务的score是否没有梯度相当于判断每个任务的子网是否没有变化
                    ...
                    #断言判断乘积后的梯度中非掩码部分的和为0，即梯度被正确掩码处理
