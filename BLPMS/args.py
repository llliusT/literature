...
def parse_arguments():
    ...
    parser.add_argument( "--superglue", action="store_true", default=False,)
    # parser.add_argument( "--server-home", required=True, type=str,default="/home/gch/project/Continual_Learning/exessnet-main", help="Home folder for the server", )
    parser.add_argument( "--server-home", type=str,default="/home/gch/project/biye_final/BLPMS", help="Home folder for the server", )
    parser.add_argument( "--data", default='/home/gch/project/biye_final/BLPMS/data', type=str, help="Location to store data", )
    ...
    parser.add_argument( "--config", type=str, default="/home/gch/project/biye_final/BLPMS/configs/transfer/config/text.yaml", help="Config file to use, YAML format" )
    ...
    parser.add_argument( "--weight-epochs", type=int, default=2, metavar="N", help="number of epochs to train weights (default: 0)", )
    # parser.add_argument( "--weight-mask-type", type=str, default="original", help="Mask type specifying which weights are updated." )
    parser.add_argument( "--weight-mask-type", type=str, default="exclusive", help="Mask type specifying which weights are updated." )
    parser.add_argument( "--ewc-lambda", type=float, default=0, metavar="LR", help="loss mixing coefficients.", )#损失混合系数
    # parser.add_argument( "--train-weight-lr", default=0.0001, type=float, help="While training the weights, which LR to use.", )
    parser.add_argument( "--train-weight-lr", default=0.001, type=float, help="While training the weights, which LR to use.", )
    parser.add_argument( "--common-coefficient", default=0.1, type=float, help="While weight-mask-type is selected as common, the coefficient of the common parameter part.", )
    parser.add_argument( "--independence-coefficient", default=0.9, type=float, help="While weight-mask-type is selected as common, the coefficient of the independence parameter part.", ) 
    # Conditioning mode
    parser.add_argument("--sim-init", type=str, default="knn_best")
    ...
    parser.add_argument("--text-tasks", default="ag,yelp,amazon,yahoo,dbpedia", type=lambda x: [str(a) for a in x.split(",")], help="Tasks for the text setup",)
    # parser.add_argument( "--emb-model", type=str, default='glove', help="" )
    parser.add_argument( "--emb-model", type=str, default='/home/gch/project/plm_cache/bert-base-uncased', help="" )
    ...
    parser.add_argument( "--log-dir", default='model_save', type=str, help="Location to logs", )
