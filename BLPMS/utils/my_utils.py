    ...
    for i, batch in enumerate(dataloader.train_loader):
        ...
        if args.text_exp:
            ...
            if args.multigpu is None:
                reps.append(model.forward(train_x, mask, True)["preds"].squeeze())
            else:
                reps.append(model.module.forward(train_x.cuda(), mask, True)["preds"].squeeze().cpu().detach())
        else:
            train_x, train_y = batch[0].to(args.device), batch[1].to(args.device)
            if args.multigpu is None:
                reps.append(model.forward(train_x, True).squeeze())
            else:
                reps.append(model.module.forward(train_x.cuda(), True)["preds"].squeeze().cpu().detach())
...
#实例化的时候传入多个文件files，调用write将files写入obj文件，调用flush刷新文件输出缓存
class Tee(object):
