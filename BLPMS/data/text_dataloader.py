...
class TextCLDataset:
    def __init__(self):
        ...
        if args.emb_model in ['glove', 'fasttext']:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained("/home/gch/project/plm_cache/bert-base-uncased")
            
        
        self.offsets = [0] * args.num_tasks#offsets这个变量一般是记录偏移量的
    ...
    def prepare_dataloaders(self):
        ...
        self.val_datasets = []
        self.test_datasets = []
        #因为是多个任务，所以dataset和dataloader都是多个
        
        use_cuda = torch.cuda.is_available()
        # kwargs = {}
        kwargs = {"num_workers": args.workers, "pin_memory": True} if use_cuda else {}
        #num_workers:4, pin_memory:True
        #pin_memory是一个特殊的内存区域，用于存储从CPU转移到GPU的数据，
        # 当将数据放入pin_memory中时，GPU会直接从pin_memory中读取数据，而不需要从CPU中读取数据，减少数据传输的开销，提高数据加载的速度。
        for i in range(task_num):
            #遍历处理每个任务的数据，存成dataset和dataloader
        ...
        if args.base_type=='multitask':
            ...
            for i in range(task_num):
                ...
                #**kwargs是一种处理关键字参数的特殊语法，允许函数或方法接收任意数量的关键字参数，并将这些参数作为字典传递给函数内部。在字典中，关键字是参数名，对应的值是传递给该参数的值。
    ...
    def get_test_data(self, data_path, max_seq_len=256,
        ...
        """
        1.获取文件中有多少行
        2.声明与行数相同的列表进行打乱，以此当做id
        3.调用get_data_by_idx获取文件中类别id和文本
        4.调用get_transformer_tokenized获取tokenizer后的文本和attention_mask
        5.声明myDataset对象
        """
    ...
    def get_transformer_tokenized(self, texts, max_seq_len):
        """
        返回tokenized的texts和attention_masks
        """
    ...
    def train_val_split(self, train_df, n_train_per_class, n_val_per_class, seed=0):
        """
        1.设置好np.random.seed(seed)和np.random.shuffle()，保证每次打乱后的train和val数据是相同的
        2.读取出train_df中有多少类别，遍历每个类别，从每个类别的idx列表中划分后n_val_per_class个数据作为val数据，前n_train_per_class个数据作为train数据
        3.打乱后返回训练数据和验证数据的idx列表        
        """
        np.random.seed(seed)#配合shuffle使用，保证每次打乱顺序后一致
        train_idxs = []
        val_idxs = []
        ...
        for cls in range(min_class, max_class + 1):
            ...
            train_pool = idxs[:-n_val_per_class]#会截取，比如一共10个，val要求4个，会把前六个截出来当train
    ...
    def get_data_by_idx(self, df, idxs):
        ...
        labels = []
        for item_id in idxs:
            labels.append(df.loc[item_id, 0] - 1)#第item_id行数据的第一列-1
            text.append(df.loc[item_id, 2])#第item_id行数据的第三列
        return labels, text
